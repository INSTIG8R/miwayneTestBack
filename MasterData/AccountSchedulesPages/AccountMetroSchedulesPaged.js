...
export default function () {
  ...
  //Account Metro Schedules Paged
  const responseAccountMetroSchedulesPaged = http.get(
  ...
  const AccountMetroSchedulesPagedstatusIs200 = check(
    responseAccountMetroSchedulesPaged,
    {
      "Status is 200": (res) => res.status === 200,
    }
  );
  // Add the check results to the responses array
  response = AccountMetroSchedulesPagedstatusIs200; //  -- Modify the condition based on your requirements
  // Traverse over the responses array and log the keys for false values
  if (response === false) {
    console.log("Key:", responseAccountMetroSchedulesPaged);
  }
