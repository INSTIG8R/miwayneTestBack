...
export default function() {
  ...
  const responseTransactionsFilter = http.post(
    "https://localhost:44367/api/Consignments/Transactions/Filter",
    JSON.stringify(payload),
    { headers }
  );

  // Check the response status code and response time
  const TransactionsFilterCountsstatusIs200 = check(
    responseTransactionsFilter,
    {
  ...
  response = TransactionsFilterCountsstatusIs200;

  // Traverse over the responses array and log the keys for false values
  if (response === false) {
    console.log("Key:", responseTransactionsFilter);
  }
