...
export default function() {
  ...
  const responseInvoiceTotalCounts = http.get(
    "https://localhost:44367/api/Consignments/Invoice/TotalCounts",
    { headers }
  );

  // Check the response status code and response time
  const InvoiceTotalCountsstatusIs200 = check(responseInvoiceTotalCounts, {
    "Status is 200": (res) => res.status === 200,
  });

  // Add the check results to the responses array
  response = InvoiceTotalCountsstatusIs200;

  // Traverse over the responses array and log the keys for false values
  if (response === false) {
    console.log("Key:", responseInvoiceTotalCounts);
  }
