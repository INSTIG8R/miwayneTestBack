...
export default function() {
  ...
  const responseAwaitingPodsTotalCounts = http.get(
    "https://localhost:44367/api/Consignments/AwaitingPod/TotalCounts",
    { headers }
  );

  // Check the response status code and response time
  const AwaitingPodsTotalCountsstatusIs200 = check(
    responseAwaitingPodsTotalCounts,
    {
  ...
  response = AwaitingPodsTotalCountsstatusIs200;

  // Traverse over the responses array and log the keys for false values
  if (response === false) {
    console.log("Key:", responseAwaitingPodsTotalCounts);
  }
