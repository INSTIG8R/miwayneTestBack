...
export default function() {
  ...
  const responseConsignmentsTotalCounts = http.get(
    "https://localhost:44367/api/Consignments/TotalCounts",
    { headers }
  );

  // Check the response status code and response time
  const ConsignmentsTotalCountsstatusIs200 = check(
    responseConsignmentsTotalCounts,
    {
  ...
  response = ConsignmentsTotalCountsstatusIs200;

  // Traverse over the responses array and log the keys for false values
  if (response === false) {
    console.log("Key:", responseConsignmentsTotalCounts);
  }
