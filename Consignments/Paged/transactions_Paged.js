...
export default function() {
  ...
  const responseTransactionsPaged = http.get(
    "https://localhost:44367/api/Consignments/Transactions/Paged?PageNumber=1&PageSize=100",
    { headers }
  );

  // Check the response status code and response time
  const TransactionsPagedCountsstatusIs200 = check(responseTransactionsPaged, {
    "Status is 200": (res) => res.status === 200,
  });

  // Add the check results to the responses array
  response = TransactionsPagedCountsstatusIs200;

  // Traverse over the responses array and log the keys for false values
  if (response === false) {
    console.log("Key:", responseTransactionsPaged);
  }
