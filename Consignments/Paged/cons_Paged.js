...
export default function() {
  ...
  const responseConsignmentPaged = http.get(
    "https://localhost:44367/api/Consignments/Paged?PageNumber=1&PageSize=100&deleted=false",
    { headers }
  );

  // Check the response status code and response time
  const ConsignmentPagedCountsstatusIs200 = check(responseConsignmentPaged, {
    "Status is 200": (res) => res.status === 200,
  });

  // Add the check results to the responses array
  response = ConsignmentPagedCountsstatusIs200;

  // Traverse over the responses array and log the keys for false values
  if (response === false) {
    console.log("Key:", responseConsignmentPaged);
  }
