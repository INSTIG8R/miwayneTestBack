...
export default function() {
  ...
  const responseAwaitingPodsPaged = http.get(
    "https://localhost:44367/api/Consignments/AwaitingPod/Paged?PageNumber=1&PageSize=100&deleted=false",
    { headers }
  );

  // Check the response status code and response time
  const AwaitingPodsPagedstatusIs200 = check(responseAwaitingPodsPaged, {
    "Status is 200": (res) => res.status === 200,
  });

  // Add the check results to the responses array
  response = AwaitingPodsPagedstatusIs200;

  // Traverse over the responses array and log the keys for false values
  if (response === false) {
    console.log("Key:", responseAwaitingPodsPaged);
  }
